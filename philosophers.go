...
// - a count of how many times he has been eating,
// - access to 2 chopsticks,
// - and a channel that allows him to send a feedback message to the Host to inform that he has finished eating
...
// - if the request to eat is accepted by the Host, the philosopher :
//   * locks the chopstick he has access to
//   * then eats during some time
//   * unlocks the chopsticks
//   * increments his count of eating
//   * and sends the feedback to the Host that he has finished eating
...
func main() {
	...
	// A channel receiving the requests from philosophers to the Host
	...
	for {
		...
		case wantToEat:
			if len(philosophersEating) == 0 {
				...
				request.philosopher.feedbackChannel <- true
...
// RejectRequestToEat sends a message back to the philosopher forbidding him to eat
